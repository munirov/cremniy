...
Вся поддержка языков идёт через единый `LanguageRegistry` — каждый язык
самодостаточно описан в своей директории в
`src/libs/CodeEditor/src/languages/<name>/` и регистрирует себя автоматически
при старте программы. Подробное руководство
по добавлению нового языка — в
[Добавление нового языка](adding_a_language_ru.md).
