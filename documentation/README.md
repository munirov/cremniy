<div align="center">

**Language / Язык:** open one section below (only one stays open in modern browsers).

</div>

# Documentation

<details name="documentation-index-lang" open>
<summary><strong>English</strong></summary>

**[Overview](EN/overview.md)** — what Cremniy is, vision, principles, built-in tools

**[Install & uninstall](INSTALL.md)** — installers, where your data lives, removal

**[Hotkeys](EN/hotkey.md)**

**[Developer guide](EN/developer_guide.md)** — run, build, add a feature

**Architecture**
- [Index](architecture/README.md)
- [BMAP](architecture/BMAP.md) — Cremniy as a whole
- [BMFP](architecture/BMFP.md) — frontend layers
- [BMBP](architecture/BMBP.md) — backend layers (mental map for the Tauri shell)
- [AGENT_CONTROL](architecture/AGENT_CONTROL.md) — capability = command rule + `window.cremniy` API
- [PLUGINS](architecture/PLUGINS.md) — how plugins work, contribution points, writing a plugin
- [DESIGN](architecture/DESIGN.md) — visual design system, palette, component patterns

**[Release guide](RELEASE.md)**

</details>

<details name="documentation-index-lang">
<summary><strong>Русский</strong></summary>

**[Обзор](RU/overview.md)** — что такое Cremniy, видение, принципы, встроенные инструменты

**[Установка и удаление](INSTALL.md)** — установщики, где данные, удаление

**[Горячие клавиши](RU/hotkey.md)**

**[Руководство разработчика](RU/developer_guide.md)** — запуск, сборка, добавление фичи

**Архитектура**
- [Индекс](architecture/README.md)
- [BMAP](architecture/BMAP.md) — Cremniy целиком
- [BMFP](architecture/BMFP.md) — слои фронта
- [BMBP](architecture/BMBP.md) — слои бэка (ментальная карта для оболочки Tauri)
- [AGENT_CONTROL](architecture/AGENT_CONTROL.md) — правило «способность = команда» + API `window.cremniy`
- [PLUGINS](architecture/PLUGINS.md) — как работают плагины, точки расширения, написание плагина
- [DESIGN](architecture/DESIGN.md) — визуальная дизайн-система, палитра, паттерны компонентов

**[Руководство по релизу](RELEASE.md)**

</details>

---

**Repository home:** [README.md](../README.md)
