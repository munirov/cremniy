...
- `plugins/<id>/index.tsx` — `default`-экспорт манифеста `PluginManifest`
  (`@shared/plugins/contributions`): `{ id, name, description?, delivery?, icon?,
  version?, author?, categories?, links?, readme?, views?, toolTabs?, centerPanels?,
  menuItems?, commands? }`. Структурные вклады — `views?` и `toolTabs?` (каждый
  `[{ id, label, railIconPath, render }]`), плюс `centerPanels?` / `menuItems?` /
  `commands?`; остальное — презентационные метаданные (см. ниже).
...
- `views: [{ id, label, railIconPath, render }]` — View-панель в боковой панели
  (активити-бар слева, рядом с Explorer/Search). `ViewContribution`; хост сводит их
  через `pluginViews()` и рисует `railIconPath` как 24×24 stroke-иконку.
- `toolTabs: [{ id, label, railIconPath, render }]` — tool-таб на правом ToolRail
  (тело — в нижнем/боковом tool-доке). `ToolTabContribution`; хост сводит их через
  `pluginToolTabs()`.
...
View-панели и tool-табы — **полноценные плагин-вклады** и уже в проде: `plugins/git/
index.tsx` отдаёт вклад `views` (Source Control), `plugins/tools/index.tsx` — `toolTabs`
(8 тулзов RE-рейла). `SidePanel.tsx` рендерит `pluginViews()`, `IdeToolDock.tsx` —
`pluginToolTabs()`. Только иконки и декорации дерева пока остаются core-швами из карты
ниже; контрибьюшн для них добавим по мере надобности.

---

## Метаданные и страница плагина

Манифест несёт не только вклады, но и **презентационные поля** — по ним строятся
строка в панели **Extensions** и страница плагина. Все поля опциональны и
аддитивны: плагин без них по-прежнему виден и работает, но с ними выглядит как в
магазине расширений. Поля (`shared/plugins/contributions.ts`):

- `icon?: string` — глиф плагина: одна 24×24 stroke-`d` (как `railIconPath` у
  view), рисуется `stroke="currentColor" fill="none"` — мелко в строке Extensions
  и крупно в шапке страницы. Стиль Lucide: тонкий, без заливки. Нет `icon` —
  рисуется нейтральный package-глиф.
- `version?: string` — версия (например `'0.1.0'`), показывается у имени.
- `author?: string` — издатель/автор, приглушённым рядом с именем.
- `categories?: string[]` — теги-категории, рендерятся чипами на странице.
- `links?: PluginLink[]` — внешние ссылки (`{ label, url }`): блок **Resources**
  на странице, открываются во внешнем браузере.
- `readme?: string` — длинное описание в **Markdown**: тело страницы плагина.
  Рендерит `MarkdownView` из внутрирепного пакета `@cremniy/markdown-view`
  (`source/frontend/packages/markdown-view/`) — полный **CommonMark + GFM** (таблицы,
  таск-листы, strikethrough, автоссылки) плюс инлайн-HTML, на npm-зависимостях
  `react-markdown` + `remark-gfm` + `rehype-raw` + `rehype-slug` (не самопальное
  подмножество). Нет `readme` — на страницу падает `description`.

**Как открывается страница.** Клик по плагину в **Extensions** ведёт на его
страницу — центр-вкладку `extensionDetails` (core-панель в `CENTER_PANELS`,
`boundary/extensions/ExtensionDetailsPanel.tsx`), в духе VS Code «Extension:
&lt;name&gt;»: шапка (иконка + имя + автор + версия + бейдж доставки + кнопка
Install/Disable) и две колонки — слева отрендеренный `readme`, справа метаданные
(**Identifier / Version / Categories / Resources / Contributes** — список того, что
плагин добавляет: его view/tool/центр-панели и команды).

Центр-панели открываются по `id` без аргументов, поэтому какой плагин показать
передаётся «сбоку»: панель Extensions кладёт id в крошечный модуль-стор
(`extensionDetailsStore.ts` → `setSelectedExtension(id)`), затем зовёт
`openPanel('extensionDetails')`; страница читает id на рендере. Одна вкладка всегда
показывает последний выбранный плагин (как один таб Extension в VS Code).

**Что это даёт контрибьютору:** заполнив `icon` + `version` + `author` +
`categories` + `links` + `readme` в своём `plugins/<id>/index.tsx`, ты описываешь
плагин прямо там — Cremniy сам нарисует строку и страницу. Эталон — `plugins/git`,
`plugins/tools`, `plugins/connections` (у каждого есть `readme` с разделами и
списком фич).
...
1. **View-пак** — панель в боковой панели (как Explorer / Search / Git). Отдаётся
   вкладом `views` в манифесте плагина (`SidePanel.tsx` сводит их в `buildViews()`
   через `pluginViews()`), попадает в активити-бар и меню «Views» с пином. Используй
   для постоянных инструментов вокруг рабочей папки.
...
**5. Вклад `views` в манифесте плагина** — `plugins/<pack>/index.tsx`. View — это
**плагин-вклад**, а не запись в ядре: `SidePanel.tsx` сводит core-вью (Explorer +
Search) с `...pluginViews()` в `buildViews()`, так что регистрировать вью в ядре не
надо — достаточно отдать `views` из манифеста (эталон — `plugins/git/index.tsx`):

```tsx
// 24×24 stroke-путь иконки рейла (один `d`-стринг, как у core-глифов)
const GIT_RAIL_ICON = 'M6 3v12 M18 9a9 9 0 0 1-9 9 …';

function GitView() {
  const workspaceRoot = useWorkspaceRoot();   // render() без аргументов — корень
  return <GitPanel workspaceRoot={workspaceRoot} />;  //   читаем из контекста
}

const git: PluginManifest = {
  id: 'git', name: 'Git',
  views: [
    { id: 'git', label: 'Source Control', railIconPath: GIT_RAIL_ICON,
      render: () => <GitView /> },
  ],
  // …centerPanels / commands
};
```

`railIconPath` рисует `SidePanel` (`PluginViewIcon`) как монохромный `currentColor`-
аутлайн на сетке 24px — отдельный компонент в `activityBarIcons.tsx` не нужен. Пин и
меню «Views» подхватятся автоматически.
...
|---|-----|------|--------------|---|
| 1 | View (боковая панель) | вклад `views` в `plugins/<id>/index.tsx` | `views: [{ id, label, railIconPath, render }]` — сводится в `buildViews()` через `pluginViews()` (`SidePanel.tsx`) | ✅ |
| 2 | Центр-панель (вкладка в Рабочем поле) | `boundary/layout/centerPanels.tsx` | запись в `CENTER_PANELS` + `openPanel(id)` | ✅ |
| 3 | Tool-таб (ToolRail / tool-док) | вклад `toolTabs` в `plugins/<id>/index.tsx` | `toolTabs: [{ id, label, railIconPath, render }]` — сводится через `pluginToolTabs()`, рендерит `IdeToolDock.tsx` (в ядре только ветка `codeEditor`); `ToolTabId` (`domain/toolTabs/toolTabId.ts`) = `string`, набор открытый | ✅ |
...
Текущая граница (вклады `contributions` — `views` / `toolTabs` / `centerPanels` /
`commands` — + `@shared/plugins` + `bridge`, плюс core-шов `#[tauri::command]`) — то,
за что динамический загрузчик встанет, не ломая уже написанные плагины.
