...
- `readme?: string` — длинное описание в **Markdown**: тело страницы плагина.
  Рендерит свой мини-рендерер (`boundary/extensions/Markdown.tsx`, без npm-зависимости;
  поддержка `#`/`##`/`###`, `**жирный**`/`*курсив*`/`` `код` ``, ``` блоки кода,
  ссылки/картинки, списки `-`/`1.`, `>` цитаты, `---`). Нет `readme` — на страницу
  падает `description`.
